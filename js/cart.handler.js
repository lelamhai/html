...
function update_cart(event,max = 0)
{
	...
	var target = $(event.target);
	console.log(target.parents("tr").find('.pre_order').val());
	if(target.parents("tr").find("input[name=quantity]").val() > 1 && target.parents("tr").find('.pre_order').val() == 1){
    	swal('Thông báo!', 'Bạn không được mua quá 1 sản phẩm cho sản phẩm này.', 'error');
    	target.parents("tr").find("input[name=quantity]").val(1);
    	return false;
    }else{
	    $.ajaxSetup({
	        headers: {
